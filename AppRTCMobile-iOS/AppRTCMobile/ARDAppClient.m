...
- (void)connectToRoomWithId:(NSString *)roomId settings:(ARDSettingsModel *)settings isLoopback:(BOOL)isLoopback {
    ...
    //        }
    //        RTCLogInfo(@"turnServers: %tu", [turnServers count]);
    //        ARDAppClient *strongSelf = weakSelf;
    //        [strongSelf.iceServers addObjectsFromArray:turnServers];
    //        strongSelf.isTurnComplete = YES;
    //        [strongSelf startSignalingIfReady];
    //    }];
    NSMutableArray *myturns = [NSMutableArray array];
    NSArray *turns = [NSArray arrayWithObjects:@"turn:171.255.193.154:4478?transport=udp" , nil];
    RTCIceServer *rtcice = [[RTCIceServer alloc] initWithURLStrings: turns  username:@"0988084021" credential:@"123456" ];
    
    //RTCIceServer *rtcice2 = [[RTCIceServer alloc] initWithURLStrings: [NSArray arrayWithObjects:@"stun:171.255.193.155", nil]];
    [myturns addObject:rtcice];
    
    [_iceServers addObjectsFromArray:myturns];
    _isTurnComplete = YES;
    
    //[self startSignalingIfReady];
    
    // Join room on room server.
    [_roomServerClient joinRoomWithRoomId:roomId isLoopback:isLoopback completionHandler:^(ARDJoinResponse *response, NSError *error) {
        ARDAppClient *strongSelf = weakSelf;
        if (error) {
        ...
        NSError *joinError = [[strongSelf class] errorForJoinResultType:response.result];
        if (joinError) {
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didCreateSessionDescription:(RTCSessionDescription *)sdp error:(NSError *)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        if (error) {
            ...
            NSError *sdpError =
            [[NSError alloc] initWithDomain:kARDAppClientErrorDomain code:kARDAppClientErrorCreateSDP userInfo:userInfo];
            [self.delegate appClient:self didError:sdpError];
            return;
        }
        __weak ARDAppClient *weakSelf = self;
        [self.peerConnection setLocalDescription:sdp completionHandler:^(NSError *error) {
            ARDAppClient *strongSelf = weakSelf;
...
- (void)peerConnection:(RTCPeerConnection *)peerConnection didSetSessionDescriptionWithError:(NSError *)error {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        // Caller khong thuc hien doan nay
        if (!self.isInitiator && !self.peerConnection.localDescription) {
            RTCMediaConstraints *constraints = [self defaultAnswerConstraints];
            __weak ARDAppClient *weakSelf = self;
            [self.peerConnection answerForConstraints:constraints completionHandler:^(RTCSessionDescription *sdp, NSError *error) {
                ARDAppClient *strongSelf = weakSelf;
...
- (void)startSignalingIfReady {
    if (!_isTurnComplete || !self.hasJoinedRoomServerRoom) {
        RTCLogError(@"[NGOCNK] Not start signaling _isTurnComplete = %d , hasJoinedRoomServerRoom = %d", _isTurnComplete, self.hasJoinedRoomServerRoom);
        return;
    }
    ...
    // Create AV senders.
    // Add media stream vao peer connection
...
- (void)processSignalingMessage:(ARDSignalingMessage *)message {
    ...
    switch (message.type) {
        ...
        case kARDSignalingMessageTypeAnswer: {
            ...
            __weak ARDAppClient *weakSelf = self;
            [_peerConnection setRemoteDescription:description completionHandler:^(NSError *error) {
                ARDAppClient *strongSelf = weakSelf;
                [strongSelf peerConnection:strongSelf.peerConnection didSetSessionDescriptionWithError:error];
            }];
            break;
...
- (void)sendSignalingMessage:(ARDSignalingMessage *)message {
    if (_isInitiator) {
        __weak ARDAppClient *weakSelf = self;
        [_roomServerClient sendMessage:message forRoomId:_roomId clientId:_clientId completionHandler:^(ARDMessageResponse *response, NSError *error) {
            ARDAppClient *strongSelf = weakSelf;
            if (error) {
                [strongSelf.delegate appClient:strongSelf didError:error];
                return;
...
- (void)registerWithColliderIfReady {
    ...
    if (!_channel) {
        ...
        if (_isLoopback) {
            _loopbackChannel = [[ARDLoopbackWebSocketChannel alloc] initWithURL:_websocketURL restURL:_websocketRestURL];
        }
    }
...
+ (NSError *)errorForJoinResultType:(ARDJoinResultType)resultType {
    ...
    switch (resultType) {
        ...
        case kARDJoinResultTypeFull: {
            error = [[NSError alloc] initWithDomain:kARDAppClientErrorDomain code:kARDAppClientErrorRoomFull userInfo:@{ NSLocalizedDescriptionKey: @"Room is full.", }];
            break;
        }
    }
    return error;
