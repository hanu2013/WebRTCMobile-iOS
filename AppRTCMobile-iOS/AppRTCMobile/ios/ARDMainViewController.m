...
@interface ARDMainViewController () <ARDMainViewDelegate,ARDVideoCallViewControllerDelegate,RTCAudioSessionDelegate>
@end
...
- (void)mainView:(ARDMainView *)mainView didInputRoom:(NSString *)room isLoopback:(BOOL)isLoopback {
    ...
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"\\w+" options:options error:&error];
    if (error) {
    ...
    ARDVideoCallViewController *videoCallViewController = [[ARDVideoCallViewController alloc] initForRoom:trimmedRoom isLoopback:isLoopback delegate:self];
