...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _connectButton.titleLabel.font = controlFont;
        [_connectButton sizeToFit];
        [_connectButton addTarget:self action:@selector(onConnect:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [_audioLoopButton addTarget:self action:@selector(onToggleAudioLoop:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews {
    ...
    CGFloat startCallTop = CGRectGetMaxY(_audioLoopButton.frame) + kCallControlMargin * 3;
    _startCallButton.frame = CGRectMake(kCallControlMargin,
                                        startCallTop,
                                        _startCallButton.frame.size.width,
                                        _startCallButton.frame.size.height);
    
    CGFloat connect =  CGRectGetMaxY(_startCallButton.frame) + kCallControlMargin * 3;
    _connectButton.frame = CGRectMake(kCallControlMargin,
                                        connect,
                                        _connectButton.frame.size.width,
                                        _connectButton.frame.size.height);
    
    CGFloat disconnect = CGRectGetMaxY(_connectButton.frame) + kCallControlMargin * 3;
    _disconnectButton.frame = CGRectMake(kCallControlMargin,
                                        disconnect,
                                        _disconnectButton.frame.size.width,
                                        _disconnectButton.frame.size.height);
}
